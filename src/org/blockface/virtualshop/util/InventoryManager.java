...
public class InventoryManager {
	...
	public ItemStack quantify(ItemStack type, boolean meta, boolean durability) {
		int amount = 0;
		Map.Entry which;
		for (Iterator<?> i = this._inv.all(type.getType()).entrySet().iterator(); i
				.hasNext(); amount += ((ItemStack) which.getValue())
				.getAmount()) {
			which = (Map.Entry) i.next();
			if (type.getType().getMaxDurability() <= 0 ? (!meta)
					&& (handleData(type.getData()) == handleData(((ItemStack) which
							.getValue()).getDurability()))
					: (durability)
							&& (type.getDurability() != ((ItemStack) which
									.getValue()).getDurability()))
				;
		}
		return new ItemStack(type.getType(), amount, type.getDurability(),
				Byte.valueOf(handleData(type.getData())));
	}
	...
	public boolean contains(ItemStack stack, boolean meta, boolean durability) {
		return ((stack.getAmount() > 0) && (quantify(stack, meta, durability)
				.getAmount() >= stack.getAmount()))
				|| ((stack.getAmount() == 0) && (quantify(stack, meta,
						durability).getAmount() >= 1));
	}
    ...
    for (int i = 0; (amount <= 0) || ((this._player == null) && (this._loc == null));) 
    { 
	...
	public ItemStack remove(ItemStack stack, boolean meta, boolean durability) {
		...
		for (Map.Entry which : this._inv.all(stack.getType()).entrySet()) {
			if (stack.getType().getMaxDurability() <= 0 ? (meta)
					|| (handleData(stack.getData()) != handleData(((ItemStack) which
							.getValue()).getDurability()))
					: (!durability)
							|| (stack.getDurability() == ((ItemStack) which
									.getValue()).getDurability())) {
				if (amount >= ((ItemStack) which.getValue()).getAmount()) {
					...
					this._inv.setItem(((Integer) which.getKey()).intValue(),
							rep);
		...
		return new ItemStack(stack.getType(), stack.getAmount() - amount,
				stack.getDurability(),
				Byte.valueOf(handleData(stack.getData())));
	}
}